...
class TestFileHandler:
    def setup_method(self):
        ...
        self.crypto_list = [CryptoElem("test1", "rot13", "encrypting"), CryptoElem("test2", "rot47", "decrypting")]
    ...
    def test_should_correctly_write_memory_buffer_to_json_file(self):
        with NamedTemporaryFile(suffix='') as temp_file:
            file_path = temp_file.name
            print(file_path)
            self.handler.folder_path = ""
            self.handler.write_to_file(self.crypto_list, file_path)
            with open(f"{file_path}.json", "r") as outfile:
                content = json.load(outfile)
                assert content ==  {"memory_buffer": [asdict(crypto) for crypto in self.crypto_list]}
